...
void display_guitar2(int new_position_us) {
	...
	if (new_position_us >= max_us){
		guitar2.note = 0;
		guitar2.posx = POSX_NO;
	} else {
		guitar2.note = 1 + (new_position_us * MAX_NOTE) / max_us ;
		guitar2.posx = POSX[guitar.note];
	}
...
void send_next(unsigned char *dataToSend,int size) {
	HAL_UART_Transmit_DMA(&huart5, dataToSend, size);
}


void receive_next(unsigned char *dataReceived) {
	HAL_UART_Receive(&huart5, dataReceived, 1, 1);
}
...
void add_notes_future(unsigned char * voice, unsigned char received_data) {
	int local_time = current_time + 30;
	//for (int i = 0; i<30; i++){
